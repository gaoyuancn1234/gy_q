...
def check_config_consistency() -> list[str]:
    ...
    for key in ('n_drop', 'vol_target', 'exec_lag'):
        bt_val = raw.get(key)
        live_val = live.get(key)
        if key == 'exec_lag':
            same = int(bt_val or 1) == int(live_val or 1)
        else:
    ...
    if preset == 'alpha158' and lag != 1:
        problems.append(
            "标签/成交分叉: alpha158 日收益标签应对 exec_lag=1")
    return problems
...
def main() -> int:
    ...
    if cfg_problems:
        ...
        print("  ✓ 实盘与回测读到的 n_drop / vol_target / exec_lag 一致")
