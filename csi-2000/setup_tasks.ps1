...
if ($WorkDir -match "csi-2000") {
    Write-Host "这是 CSI300 的 TradingSystem-* 任务脚本。" -ForegroundColor Red
    Write-Host "在 csi-2000 目录跑会撞原树定时任务、可能写错数据源。"
    Write-Host "CSI2000 用: powershell -File setup_csi2000_tasks.ps1"
    exit 2
}
if (-not $PythonExe) {
