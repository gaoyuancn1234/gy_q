...
> **当前任务只针对 CSI2000（本目录）。不要改 CSI300 实盘。**
> 原树 `gy_q/trading_framework`、持仓、定时任务、`cn_data_bs` 一律不动。
> 行情中台只走 Tushare，不必为 300 保留新浪/BaoStock。
>
> **CSI2000 现行状态、配置、未闭环项见 `doc/README.md`。**
> 下文大段仍是 CSI300 历史（绩效表、TopK16、止损/vol_target 8%）。
> 研究方法（8 相位、对账、标签必须等于成交）仍适用；那些数字和参数不是
> 中证2000 的约束。
...
|---|---|---|---|---|---|---|
| 段1 2022-05~2023-12 (样本外) | 0.780 | 0.331 | 0.271 | +13.71% | -11.29% | **+28.15%** |
| 主段 2024-01~2026-09 | 1.205 | 0.227 | 0.966 | +43.79% | -9.66% | +9.49% |

复现: `python run_phase_test.py --phases 8`(主段) /
`python run_phase_test.py --start 2022-05-04 --end 2023-12-29 --tag 段1 --pred-tag pre2024`

**vs 沪深300**: 熊市极强 (段1 超额 +28.15%)，牛市可能跑输 —— 主段超额均值
+9.49%，**最差相位 -1.93%(跑输指数)**。
...
> 2026-09-06 再次全部重测: 仓位分配此前两边各写一份、四条语义都不同
> (太贵剔除/不足一手/交易成本/跳过的钱是否重分)，模拟盘系统性少投一部分钱。
> 统一到 rebalance_rules.allocate_buys 后主段 1.161->1.205、段1 0.856->0.780
> (回测投满资金后，熊市段回撤从 -8.80% 深到 -11.29%)。对账已补上仓位维度。

### vol_target=8% 的配对检验 (2026-09-06)

`python run_param_sweep.py --param vol_target --values 0.08 0 0.06 0.10`
8 相位 × 2 段。基线 8%。

    主段          ΔSharpe   配对t   胜出    Δ回撤     Δ超额
    关闭           -0.008   -0.17   4/8   -1.47%   +7.78%
    6%             -0.059   -3.39   2/8   +0.58%   -6.36%
    10%            -0.007   -0.35   3/8   -0.57%   +2.65%

    段1           ΔSharpe   配对t   胜出    Δ回撤     Δ超额
    关闭           +0.189   +2.22   5/8   +0.10%   +5.41%
    6%             +0.031    0.63   5/8   +0.56%   -0.12%
    10%            +0.056    1.46   6/8   -0.44%   +1.52%

**唯一统计显著的结论: 6% 明显更差** (主段 t=-3.39)。不要下调。

**8% 本身证据不足**: 关掉它之后 Sharpe 两段都不差或更好、超额两段都更好
(+7.78% / +5.41%)，而回撤只在主段改善 1.47 个百分点、段1 几乎不变。
即这道风控花掉 5~8 个百分点的超额，只换回主段 1.5 个百分点的回撤。

**当前决定: 维持 8% 不动。** 依据是事先定的判据 —— "|t|>=2 且两段方向一致
才改"，而主段是噪声、段1 只有 t=2.22 (n=8, p≈0.06)，不足以改动实盘参数。
回撤保护的价值也不完全体现在 Sharpe 里。

**但 CLAUDE.md 长期写的"价值主要在更小回撤"需要修正**: 这个回撤是用相当多
的超额换来的，且熊市段没换到。会推翻本结论的证据: 相位加到 16 个后段1 的
t 仍 >=2，或换一个区间(如 2020-2021)复现同方向。
...
### 两个回测引擎给出相反结论 (2026-09-07 实测)

同一份 210 因子预测、同一个基准(沪深300 +34.31%):

    引擎                      总收益    超额     Sharpe   持仓   交易   敞口
    hifi(VolTargetBacktester) 32.18%   -2.13%   1.060   15.5   709   83.3%
    paper_trader              47.87%  +13.56%   1.257   15.0   685    --

持仓只数、交易笔数、敞口都接近，**差距来自持有了不同的股票**。分歧集中在
2024Q1(+2.84pp)、2024Q3(+2.76pp)、2026Q3(+5.19pp)，中间近两年几乎不动 ——
不是均匀的成本拖累。

成因是第 4 次出现的"同一条规则两份实现": hifi 内联了自己的调仓逻辑，
卖出不走 `rebalance_rules.select_sells`、买入不走 `allocate_buys`、
也没有自适应 TopK(固定 topk，实盘按信号质量取 12/16/16)。

**结论: `run_rolling_benchmark` 的表只能用于因子集/模型之间横向比较，
不能当实盘绩效。** 已在表头加警示。报绩效一律用 `run_phase_test.py`。
未做的事: 把 VolTargetBacktester 的调仓改为复用 rebalance_rules
(约 120 行内联逻辑，需单独验证后再动)。
...
### 安全网本身会坏，而且坏了之后每次都报"通过" (2026-09-14)

`reconcile.py` 是这套系统的安全网，文件开头写着"这些 bug 不需要被预测，
只需要被对账"。当天发现**它自己带着一个 2026-09-10 就已经在别处修好的 bug**：
取价用 `D.instruments('csi2000')`（时点成分股），而 `paper_trader.replay`
早已改成 `'all'`。后果与当初一模一样 —— 被调出指数的持仓取不到价，
`if inst not in day_close.index: continue` 直接跳过，**需要卖的恰恰是取不到
价的那些**，于是永远卖不掉、仓位被永久占住。

实测 2026-09-04 那个调仓日：98 只持仓里 31 只早已调出指数（在 csi2000 的
1999 只里取价命中 0/8，在 'all' 的 2906 只里命中 8/8），n_drop=20 的卖出
名额全被它们占满，82 个调仓日里 78% 如此；最早的入场日期是 2024-02-05，
压了两年半。**也就是说对账一直在拿一个从 2024 年中就冻住的组合去验证实盘
路径，而且每次都报"分叉 0"。**

修完之后三个数同时变好，互相印证（这种互证比单个指标可信）：

```
无分数占满 n_drop        108 次 -> 0 次
qlib TopkDropout 重合度    48% -> 97%
逐日订单分叉                 0 -> 0
```

**教训**：同一个 bug 在两个文件里各写一遍时，修了一处不等于修完。
`grep` 那句注释里的关键词（这次是 `D.instruments(`）应该是修复动作的一部分。
**"安全网通过"这个信号的分量，取决于安全网自己有没有被检查过。**

### 测量代码的 bug 会伪装成"好结果" (2026-09-14)

当天一天内踩到三次，共同点是**结果看起来是好的**，只有第三次是工具自己喊出来的：

1. **自己跟自己比**。快照 vs 官方日线的比对函数用
   `join(..., rsuffix="_off")`，之后仍按原名取值 `m["open"]` —— 两边都指向
   快照。五个字段误差恒为 0.0、"100% 落在 0.1% 内"，看着像完美一致。
   只有 volume 因列名没撞（`volume` vs `vol`）是真的。**差点据此宣布验证通过。**
   修完是真比对，结论恰好也是精确相等 —— 但那是运气，不是当时的证据。
2. **`$?` 取到的是管道最后一个命令的退出码**。`reconcile.py ... | grep | tail`
   之后 `echo $?` 报的是 `tail` 的 0。而 reconcile 的真实输出全在
   `logs/reconcile.log`，又被后跑的冒烟（短窗对账）覆盖 —— 等于跑了一次
   全量对账，然后把证据自己删了，还对着一个假的退出码说"通过"。
3. **重建输入时漏字段**。把 `select_sells` 的次级键改成 `entry_date` 后，
   对账立刻报 54 个调仓日分叉 —— 根因是它把实盘侧持仓重建成 `{code: {}}`，
   `entry_date` 全空，实盘侧退回代码序而模拟盘侧用真实日期。
   **决策快照必须带上实盘路径会读的每一个字段**，否则比出来的是重建缺陷。

**教训**：验证代码和被验证代码一样会错，而且它的错更难发现 ——
因为它报错时你会去查被测对象，它报通过时你根本不查。
**长任务不要把 stdout 接进 `tail`**（会缓冲到结束，中途看不到进度，
当天一个 34 分钟的扫描全程盲跑）；让它落文件，再去 tail 文件。

### 跑实验前先翻 `factor_lab/results/` (2026-09-14)

当天花 40 分钟扫 `rebalance_every`，又提议再花一个多小时扫
`rebalance_every + n_drop` 二维 —— 而 `sweep_rebalance_every+n_drop_持仓期.json`
（2026-09-13）就躺在结果目录里，测的正是同一件事，结论一致且更完整
（它控制了换手率恒定）。**结果文件是纳入版本管理的，扫之前先 `ls`。**

### 简化计算可能描述的是一个不存在的策略 (2026-09-14)

为回答"标签(隔夜)与持仓期(8天)错配有多严重"，算了一张各持有期的净超额表：
隔夜年化 +67.8%、持有 8 日只有 +11.7%，据此推断"82% 的 alpha 在第一个隔夜
赚到，持有 8 天在浪费信号"。

算术没错，但那张表假设**每期全额换仓**。真实策略有 `n_drop=20`，每次只换
20/100。两次八相位扫描（2026-09-13 的二维 + 2026-09-14 的一维）都否定了
提高频率，而且 9-13 那次**控制了换手率恒定**（8+20 / 4+10 / 2+5 / 1+3），
结论依然是频率越高越差且单调（1+3 t=-2.83，1+20 t=-4.91）——
**所以不是成本问题，是这个方向本身不成立。**

**教训**：脱离约束（换手上限、整手、资金规模、敞口缩放）的收益估算，
描述的是另一个策略。数字越诱人越要先问"这是在算哪个策略"。
**"错配"仍然成立，但唯一没试过的解法是让标签对齐持有期**（8 日前瞻收益
重训），不是提高交易频率。届时 IC 会明显低于隔夜标签的 0.227，
那是对齐用途的正常结果，判断要看净超额和 DSR，不能看 IC。
