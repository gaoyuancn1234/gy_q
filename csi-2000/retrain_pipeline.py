...
def refresh_data(end_date: str = None):
    """刷新行情数据. 中证2000 走 Tushare, 其余沿用 BaoStock/新浪."""
    print("\n" + "=" * 60)
    print("Step 1: 刷新行情数据")
    print("=" * 60)
    ...
    if universe == "csi2000":
        import subprocess
        from qlib_paths import TUSHARE_START
        cmd = [
            sys.executable, "-X", "utf8",
            "-m", "qlib_engine.data_setup_tushare",
            "--universe", universe,
            "--target_dir", str(Path(data_dir).expanduser()),
        ...
        setup_qlib_data(target_dir=data_dir, end_date=end_date,
                        universe=universe)
