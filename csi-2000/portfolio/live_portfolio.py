...
不依赖 qlib.init() — 价格和名称走 Tushare (未复权, 与成交价同口径)。
"""
import json
import time
...
def qlib_to_bao(code: str) -> str:
    """SH600036 → sh.600036 (历史命名, 不再连 BaoStock)。"""
    prefix = code[:2].lower()
    num = code[2:]
    return f"{prefix}.{num}"
...
def _fetch_names_tushare(codes: list) -> dict:
    """Tushare stock_basic 一次拉全市场名称。"""
    ...
    if df is None or df.empty:
        return {}
    table = {
        _ts_to_qlib(r.ts_code): r.name for r in df.itertuples(index=False)
    }
    return {c: table[c] for c in codes if c in table}


def get_stock_names(codes: list | None = None) -> dict:
    """股票中文名映射 {SH600036: '招商银行'}

    名字只是显示用, 取不到就回落成代码本身, 绝不阻塞下单。
    """
    ...
    try:
        _stock_names_cache.update(
            run_with_timeout(
                lambda: _fetch_names_tushare(want),
                NAME_FETCH_TIMEOUT,
                "Tushare 取名称",
            )
        )
        ...
        print(
            f"[stock_names] Tushare 取名失败 "
            f"({type(e).__name__}): {e}，用代码代替"
        )
    return _stock_names_cache
...
def _price_trade_date() -> str:
    """取价用的交易日: 收盘后用今天, 否则用上一交易日。"""
    from datetime import date
    from data_hub.calendar import open_days
    days = open_days()
    if not days:
        raise RuntimeError("没有交易日历, 无法取价")
    today = date.today().strftime("%Y-%m-%d")
    now = datetime.now()
    closed = now.hour * 60 + now.minute >= 15 * 60 + 5
    if today in days and closed:
        return today
...
def _fetch_prices_tushare(instruments: list) -> dict:
    """Tushare 未复权收盘价, 与成交截图原始价同口径。

    盘中实时行情需单独付费, 15000 积分不含。18:00 路径用 daily。
    """
    from data_hub.client import call, pro_api
    want = {_qlib_to_ts(c) for c in instruments}
    ymd = _price_trade_date().replace("-", "")
    client = pro_api()
    df = call(
        client.daily, trade_date=ymd, fields="ts_code,close",
    )
    if df is None or df.empty:
        return {}
    prices = {}
    for row in df.itertuples(index=False):
        if row.ts_code not in want:
            continue
        try:
            px = float(row.close)
        ...
        if px <= 0:
            continue
        prices[_ts_to_qlib(row.ts_code)] = px
    return prices
...
def get_current_prices(instruments: list) -> tuple:
    ...
    Tushare 未复权日线 → 本地当日缓存。不再走新浪/BaoStock。
    """
    if not instruments:
        return {}, False
    try:
        prices = run_with_timeout(
            lambda: _fetch_prices_tushare(instruments),
            PRICE_FETCH_TIMEOUT,
            "Tushare 取价",
        )
        if prices:
            ...
            return prices, False
        print("[get_prices] Tushare 返回空，转本地缓存")
    except Exception as e:
        print(
            f"[get_prices] Tushare 失败 ({type(e).__name__}): {e}, "
            "尝试本地缓存..."
        )
...
def _get_vol_target_config() -> dict:
    ...
    try:
        ...
        return {'vol_target': None, 'n_drop': None, 'exec_lag': 1}
