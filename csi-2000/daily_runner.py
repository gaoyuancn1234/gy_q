...
CSI2000 日常请用: python -m daily run
本文件保留给影子/实验盘子命令, 以及 daily.py 复用的函数。

用法:
    python -m daily run
    python daily_runner.py --dry-run
    python daily_runner.py --force
...
def refresh_daily_data() -> bool:
    ...
    from qlib_paths import TUSHARE_START, data_refresh_chain
    from data_hub.paths import TUSHARE_ONLY
    from data_hub.guard import refuse_non_tushare
    for label, module in data_refresh_chain(universe):
        if universe in TUSHARE_ONLY and "sina" in module:
            log.error("中证2000 禁止新浪刷新, 已跳过")
            continue
        if "sina" in module or module.endswith("data_setup"):
            try:
                refuse_non_tushare(universe, data_dir)
        ...
        try:
            ...
            if module == "qlib_engine.data_setup_tushare":
                cmd += ["--start_date", TUSHARE_START]
...
def generate_and_push(dry_run: bool = False, degraded: list = None,
                     signal: dict = None, prices: dict = None):
    """生成信号 → 推送飞书。

    signal/prices 由 14:45 auction 注入时, 不读隔日 pkl、不拉收盘日线。
    """
    from factor_lab.signal_generator import SignalGenerator
    ...
    from market_calendar import next_open_day

    # 预检查: 预测缓存文件是否存在
    if signal is None and not PRED_PKL.exists():
    ...
    if not prices:
        log.error("获取价格失败")
        push_feishu("❌ 获取价格失败，请检查 Tushare 日线/缓存", dry_run)
        from notifications import write_notification
        write_notification("signal", "获取价格失败", "Tushare 无价", "error")
        return
    if from_cache:
        log.warning("使用缓存价格 (Tushare 当日日线未出)")
        if degraded is None:
            degraded = []
        degraded.append("价格来自缓存(Tushare当日未出)")
    ...
    if is_rebalance:
        ...
        lag = get_exec_lag()
        execute_on = (
            signal['date'] if lag == 0
            else next_open_day(signal['date'])
        )
        ...
        holdings['exec_lag'] = lag
        holdings['execute_on'] = execute_on
        save_live_holdings(holdings)
        log.info(
            f"成交时钟 exec_lag={lag} execute_on={execute_on}"
        )
