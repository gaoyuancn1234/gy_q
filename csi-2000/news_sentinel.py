...
class NewsSentinel:
    ...
    def fetch_macro_news(self, target_date: str = None) -> dict:
        ...
        try:
            df = call(client.cctv_news, date=ymd)
        ...
        try:
            ...
            print(f"  [sentinel] 财联社: {len(result['cls'])} 条")
        except Exception as e:
    ...
    def fetch_stock_news(self, stock_codes: list,
        ...
        for code in stock_codes:
            ...
            try:
                ...
                print(f"  [sentinel] {code}: {len(news_list)} 条公告")
            except Exception as e:
                print(f"  [sentinel] {code} 公告获取失败: {e}")
                result[code] = []
            time.sleep(SLEEP)
