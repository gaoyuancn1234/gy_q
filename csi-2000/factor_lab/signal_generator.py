...
class SignalGenerator:
    ...
    def __init__(self, config_path: str = 'config/signal_config.yaml',
                 pred_tag: str = None, preset: str = None):
        # preset 覆盖: 不同因子集训练出的预测存在不同文件名里
        # (如 alpha158_selected=188因子 / alpha158_val=210因子)。
        # 只改 pred_tag 不够 —— 文件名里 preset 也是一段。
        self.pred_tag = pred_tag
        self.preset_override = preset
    ...
    def load_predictions(self) -> pd.Series:
        ...
        _preset = getattr(self, 'preset_override', None) or cfg['preset']
        pkl_name = f"{cfg['rolling_config']}_{_preset}_{cfg['model']}{_tag}.pkl"
    ...
    def get_signal(self, date: str = None) -> dict:
        ...
        # 与 paper_trader 同一套开关: none/off 时用配置 topk, 不读质量分.
        _adaptive = str(self.config.get('adaptive_strategy') or '').strip().lower()
        _adaptive_on = _adaptive not in ('', 'none', 'off', 'false', '0')
        if _adaptive_on:
            quality = self.load_quality_score()
            ...
            quality = None
            regime = 'normal'
            effective_topk = self._cap_topk(int(self.config.get('topk', 16)))
    ...
    def signal_from_scores(self, day_signal: pd.Series, date: str) -> dict:
        """用外部日截面分数生成与 get_signal 同结构的指令。

        14:45 截断出分走这条, 不读隔日 pkl。
        """
        day_signal = day_signal.sort_values(ascending=False)
        _adaptive = str(
            self.config.get('adaptive_strategy') or ''
        ).strip().lower()
        _adaptive_on = _adaptive not in (
            '', 'none', 'off', 'false', '0',
        )
        if _adaptive_on:
            quality = self.load_quality_score()
            regime = self._get_regime(pd.Timestamp(date))
            effective_topk = self._cap_topk(TOPK_BY_REGIME[regime])
        else:
            quality = None
            regime = 'normal'
            effective_topk = self._cap_topk(
                int(self.config.get('topk', 16)),
            )
        target_stocks = list(day_signal.head(effective_topk).index)
        q_val = None
        if quality is not None:
            valid_q = quality[quality.index < pd.Timestamp(date)]
            if len(valid_q) > 0:
                q_val = float(valid_q.iloc[-1])
        health = self._check_signal_health(
            day_signal, pd.Timestamp(date),
        )
        return {
            'date': str(date)[:10],
            'regime': regime,
            ...
            'target_stocks': target_stocks,
            'scores': day_signal.to_dict(),
            'current_window': self._find_current_window(
                pd.Timestamp(date),
            ),
            'health': health,
        }
