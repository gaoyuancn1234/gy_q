...
def train_window(dataset, model_name: str, prev_best_iters: list,
                 ...
                 save_meta: dict | None = None) -> tuple:
    ...
    save_dir 非空时把 LightGBM booster 和 RobustZScoreNorm 统计落盘,
    给 14:45 截断出分。滚动训练每个窗口覆盖写, 留下最后一窗。
    ...
    for i, sd in enumerate(seeds):
        ...
        if save_dir is not None and model_name == "LightGBM":
            from factor_lab.lgb_store import save_booster
            save_booster(model, save_dir, i)
        if len(seeds) > 1:
            print(f"      [seed {i+1}/{len(seeds)}] best_iter={best}")
        del model
    if save_dir is not None and model_name == "LightGBM":
        from factor_lab.lgb_store import write_infer_meta
        write_infer_meta(dataset, save_dir, save_meta or {})
...
def run_rolling_single(preset: str, model_name: str,
    ...
    for w in windows:
        ...
        from factor_lab.lgb_store import booster_dir as _booster_dir
