...
执行模型:
- exec_lag=1 (默认): T 收盘出信号, T+1 收盘成交
- exec_lag=0 (路线 B): T 14:45 出信号, T 14:50 收盘竞价成交
...
class PaperTrader:
    """模拟盘引擎 — 收盘价成交。exec_lag=1 隔日, 0 当日。"""
    ...
    def __init__(self, config_path: str = 'config/signal_config.yaml',
        ...
        self.sg = SignalGenerator(config_path, pred_tag=pred_tag, preset=preset)
    ...
    def _fill_pending(self, date_str: str, day_close: pd.Series,
                      prev_close_map: dict, cfg: dict) -> int:
        """按收盘价撮合 pending。返回成交笔数。"""
        ...
        if pending['buys']:
            ...
            available_cash = self.state['cash'] * 0.99
            # 波动率目标: 缩减本次可投入的资金。
            #
            # 2026-09-05 修复。原先 exposure 只存进 pending['buys'] 的
            # weight 里, 分配式 alloc = cash * (weight / sum(weights)),
            ...
            if total_weight > 0:
                ...
                for inst in pending['buys']:
                    ...
                    if inst in prev_close_map and prev_close_map[inst] > 0:
                        hi = price_limit(inst)
                        if (day_close[inst] / prev_close_map[inst] - 1
                                > hi):
                            continue
                    buyable.append(inst)
                ...
                alloc = allocate_buys(
                    buyable, bprices, available_cash,
                ...
                for inst, a in alloc.items():
                    ...
                    self._append_trade(
                        date_str, 'BUY', inst, shares, price,
    ...
    def update_daily(self, date: pd.Timestamp, prices_df: pd.DataFrame,
                     ...
                     signal: pd.Series, quality_score: pd.Series | None,
        ...
        # --- 1. 执行挂起的交易 (lag=1 是昨单) ---
        n_trades += self._fill_pending(
            date_str, day_close, prev_close_map, cfg,
        )
        ...
        if day_idx % rebal == 0 and date in signal_dates:
            ...
            regime = 'normal'
            if _adaptive_on and quality_score is not None:
            ...
            if date in signal_dates:
                ...
                if '$amount' in day_data.columns:
                    amounts = day_data['$amount'].dropna().to_dict()
                target_list = pick_liquid(
                    list(day_signal.index), amounts, effective_topk,
                    cfg.get('min_amount'),
                )
                target_stocks = set(target_list)
                ...
                self.last_decision = {
                    ...
                    'regime': regime,
                    'target_stocks': list(target_list),
                    'scores': day_signal.to_dict(),
    ...
    def replay(self, start_date: str = None, end_date: str = None,
        ...
        # 都必须能估值、能卖出。
        prices = D.features(D.instruments('all'), ['$close', '$amount'],
                            start_time=start, end_time=end)
        prices = prices.swaplevel().sort_index()

        signal = self.sg.load_predictions()
        _ad = str(cfg.get('adaptive_strategy') or '').strip().lower()
        quality = None
        if _ad not in ('', 'none', 'off', 'false'):
